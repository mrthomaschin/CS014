	...
	bool erase(const T& x) 
	{
		...
		if ((*p)->left == NULL && (*p)->right == NULL) 
		{
			...
			if ((*p)->left == NULL || (*p)->right == NULL) 
			{
				...
				if((*p)->right == NULL)
				{
					Node<T>* temp = *p;	//memory allocation
					(*p) = (*p)->left;
					delete temp;
				...
				if((*p)->left && (*p)->right)
				{
					...
					Node<T>* temp = (*p)->right;
					T val;						//memory allocation

					while(temp->left != NULL)	//find smallest value in right subtree
						temp = temp->left;
			
					val = temp->value;			//assign value before erasing
					erase(temp->value);
					(*p)->value = val;

					//(*p)->value = temp->value;   Does not work, floating pointers
					//erase(temp->value);
