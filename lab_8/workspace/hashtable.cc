...
public:
    Hashtable(int n, HashFunction<K>* _h, ResolutionFunction* _f) : numCollisions(0), hash(_h), f(_f) {
        ...
        while(GoodPrimeNumbers[x] && !found)
        {
            if(GoodPrimeNumbers[x] > n)
            {
                c = GoodPrimeNumbers[x];
                found = true;
            }
            x++;
        }
    ...
    void insert(const K& key) 
    {
        ...
        unsigned int h = (*hash)(key) % buckets.size();
        int i = 0;

        while(status.at(h) != EMPTY)
        {
            h = ((*f)(i) + h) % buckets.size();
            i++;
        }

        status.at(h) = OCCUPIED;
        buckets.at(h) = key;
    }
    ...
    bool search(const K& key) {
        ...
        while(status.at(h) != EMPTY)
        {
            if(status.at(h) == OCCUPIED && buckets.at(h) == key)
                return true;
